...
class App extends React.Component {
  ...
  render() {
    let symbols = ["AC","/", "*", "-","7", "8", "9", "+","4", "5", "6", "=","1", "2", "3", "."];
    let buttons = [];
    let j = 0;
    let k = 0;
    for (let i = 0; i < 4; i++) {
      let columns = [];
      for (j; j < 4 + k; j++) {
        columns.push(<button className="col" onClick={this.handleclick} id={symbols[j]}>{symbols[j]}</button>)
      }
      k = j;
      buttons.push(
    ...
    return (
      <div className="calculator d-flex align-items-center flex-column">
        ...
        <div className="calculator-keys d-flex flex-column justify-content-center">
          {buttons}
          <div className="row"><button className="zero" onClick={this.handleclick} id="0">0</button></div>
        </div>
      </div>
    );
  }
